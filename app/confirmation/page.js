    ...
    if (order.isLoaded){
        return(
            ...
            <div className="flex justify-start item-start space-y-2 flex-col ">
                <h1 className="text-3xl lg:text-4xl font-semibold leading-7 lg:leading-9 dark:text-white text-gray-800">Order #{order.orderDetails.number}</h1>
                <p className="text-base font-medium leading-6 dark:text-white text-gray-600">{order.orderDetails.date_created}</p>
            </div>
                    ...
                    <div className="flex flex-col justify-start items-start dark:bg-slate-800  bg-gray-50 rounded-lg px-4 py-4 md:py-6 md:p-6 xl:p-8 w-full">
                        ...
                        <div className="mt-4 md:mt-6 flex  flex-col md:flex-row justify-start items-start md:items-center md:space-x-6 xl:space-x-8 w-full ">
                            <div className="pb-4 md:pb-8 w-full md:w-40">
                                <img className="w-full hidden md:block" src={order.orderDetails.items[0].product.images[0].file.url} alt="dress" />
                                <img className="w-full md:hidden" src={order.orderDetails.items[0].product.images[0].file.url} alt="dress" />
                            </div>
                            <div className="border-b border-gray-200 md:flex-row flex-col flex justify-between items-start w-full  pb-8 space-y-4 md:space-y-0">
                                <div className="w-full flex flex-col justify-start items-start space-y-8">
                                    <h3 className="text-xl xl:text-2xl font-semibold leading-6 dark:text-white text-gray-800">{order.orderDetails.items[0].product.name}</h3>
                              
                                </div>
                                <div className="flex justify-between space-x-8 items-start w-full">
                                    <p className="text-base dark:text-white xl:text-lg leading-6">
                                        ${order.orderDetails.items[0].price}
                                    </p>
                        ...
                        <div className="flex flex-col px-4 py-6 md:p-6 xl:p-8 w-full dark:bg-slate-800 bg-gray-50 rounded-lg space-y-6   ">
                            ...
                            <div className="flex justify-center items-center w-full space-y-4 flex-col border-gray-200 border-b pb-4">
                                <div className="flex justify-between  w-full">
                                    ...
                                    <p className="text-base leading-4 dark:text-white text-gray-600">${order.orderDetails.sub_total.toFixed(2)}</p>
                                </div>
                                <div className="flex justify-between  w-full">
                                    <p className="text-base leading-4 dark:text-white text-gray-800">Taxes</p>
                                    <p className="text-base leading-4 dark:text-white text-gray-600">${order.orderDetails.tax_total.toFixed(2)}</p>
                                </div>
                                <div className="flex justify-between items-center w-full">
                                    <p className="text-base leading-4 dark:text-white text-gray-800">
                                        Discount 
                                    </p>
                                    <p className="text-base leading-4 dark:text-white text-gray-600">${order.orderDetails.discount_total.toFixed(2)}</p>
                                </div>
                                <div className="flex justify-between items-center w-full">
                                    <p className="text-base leading-4 dark:text-white text-gray-800">Shipping</p>
                                    <p className="text-base leading-4 dark:text-white text-gray-600">${order.orderDetails.shipment_total.toFixed(2)}</p>
                                </div>
                            </div>
                            <div className="flex justify-between items-center w-full">
                                <p className="text-base font-semibold leading-4 dark:text-white text-gray-800">Total</p>
                                <p className="text-base font-semibold leading-4 dark:text-white text-gray-600">${order.orderDetails.grand_total.toFixed(2)}</p>
                            </div>
                        </div>
                    ...
                    <div className="flex  flex-col md:flex-row xl:flex-col justify-start items-stretch h-full w-full md:space-x-6 lg:space-x-8 xl:space-x-0 ">
                        <div className="flex flex-col justify-start items-start flex-shrink-0">
                            ...
                            <div className="flex justify-center  md:justify-start items-center space-x-4 py-4 border-b border-gray-200 w-full">
                                ...
                                <p className="cursor-pointer text-sm leading-5 dark:text-white text-gray-800">{order.orderDetails.account.email}</p>
                            </div>
                        </div>
                        <div className="flex justify-between xl:h-full  items-stretch w-full flex-col mt-6 md:mt-0">
                            <div className="flex justify-center md:justify-start xl:flex-col flex-col md:space-x-6 lg:space-x-8 xl:space-x-0 space-y-4 xl:space-y-12 md:space-y-0 md:flex-row  items-center md:items-start">
                                ...
                                <div className="flex justify-center md:justify-start  items-center md:items-start flex-col space-y-4">
                                    ...
                                    <p className="w-48 lg:w-full xl:w-48 text-center md:text-left text-sm leading-5 dark:text-white text-gray-600">Method: {order.orderDetails.method}</p>
                                    </div>
